        ...
        <div class="row px-xl-5">
            <div class="col-12">
                <nav class="breadcrumb bg-light mb-30">
                    <a class="breadcrumb-item text-dark" href="#">Home</a><?php 
                  $id=$_GET['id'];
                  $sql = "SELECT id,pname from product where id=:id";
                  ...
                  { ?>
                    <a class="breadcrumb-item text-dark" href="#"><span style="color:red"><?php echo htmlentities($result->pname);}}?></span></a>
        ...
        <div class="row px-xl-5">
            <div class="col-lg-8 table-responsive mb-5">
                <table class="table table-light table-bordered table-hover text-center mb-0">
                    ...
                    <?php  
$id=intval($_GET['id']);                     
    $sql="SELECT services.*, product.pname FROM services INNER JOIN product on product.id=services.pid  AND services.pid=:id ORDER BY services.id DESC LIMIT 50";              
...
$id=intval($_GET['id']);
$status=1;
                     $sql = "SELECT * FROM services where services.pid=:id and services.status=:status";                
                                     $query = $dbh->prepare($sql);
                                     $query->bindParam(':id', $id, PDO::PARAM_STR);
